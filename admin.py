...
class DishAdmin(ModelView, model=Dish):
    ...
    column_labels = {
        ...
        Dish.name_fr: "Plat",
        Dish.name_en: "Nom (anglais)",
        Dish.short_fr: "Description courte (français)",
        Dish.short_en: "Description courte (anglais)",
        Dish.price: "Prix",
        Dish.price_label_fr: "Libellé de prix (français)",
        Dish.price_label_en: "Libellé de prix (anglais)",
        Dish.composition_fr: "Composition (français, JSON)",
        Dish.composition_en: "Composition (anglais, JSON)",
        Dish.allergens_fr: "Allergènes (français, JSON)",
        Dish.allergens_en: "Allergènes (anglais, JSON)",
        Dish.customization_rules: "Règles de personnalisation (JSON)",
        Dish.category: "Catégorie",
        Dish.is_available: "Disponibilité",
        Dish.hidden: "Masqué du site",
    }
...
def setup_admin(app):
    ...
    async def admin_upload_image(request: Request, file: UploadFile = File(...)):
        ...
        if not content_type.startswith("image/"):
            return JSONResponse(
                {"error": "Le fichier doit être une image."}, status_code=400
            )
        ...
        # Nom de fichier propre : minuscules, sans espaces/accents ambigus,
        # préfixé d'un identifiant court pour ne jamais écraser un fichier
        # existant (deux plats ne peuvent donc jamais entrer en conflit).
        stem = os.path.splitext(original_name)[0]
        stem = re.sub(r"[^a-zA-Z0-9\-]+", "-", stem).strip("-").lower() or "plat"
        filename = f"{uuid.uuid4().hex[:8]}-{stem}{ext}"
        ...
        if len(contents) > MAX_SIZE:
            return JSONResponse(
                {"error": "Image trop volumineuse (8 Mo maximum)."}, status_code=400
            )
